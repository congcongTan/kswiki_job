...
$(document).swipeLeft(function(){
	...
	//存在先动
	if( $('.page-' + now.row + '-' + (now.col + 1)).length > 0 ){
		now.row = last.row;
		now.col++;
		pageMove(towards.left);
	}
	//第一最后一页没得翻
	// if (last.row>1 && last.row<5) { now.row = last.row; now.col++; pageMove(towards.left);}
...
$(document).swipeRight(function(){
	...
	// if (last.row>1 && last.row<5) { now.row = last.row; now.col--; pageMove(towards.right);}
	//存在先动
	if( $('.page-' + now.row + '-' + (now.col - 1)).length > 0 ){
		now.row = last.row;
		now.col--;
...
function pageMove(tw){
	...
	setTimeout(function(){
		...
		$(lastPage).find(".m").addClass("hide");

		$(nowPage).addClass('page-current');
		$(nowPage).removeClass(inClass);
		$(nowPage).find(".m").removeClass("hide");
